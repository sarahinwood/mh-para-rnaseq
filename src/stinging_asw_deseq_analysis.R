...
res_group <- results(dds_group, contrast = c("group", "Abdomen_m30", "Abdomen_Control"), lfcThreshold = 1, alpha = 0.1)
##Order based of padj
m30_ordered_res <- res_group[order(res_group$padj),]
##Make data table and write to output
m30_ordered_res_table <- data.table(data.frame(m30_ordered_res), keep.rownames = TRUE)
##filter for sig DEGs
m30_ordered_sig_res_table <- subset(m30_ordered_res_table, padj < 0.05)
fwrite(m30_ordered_sig_res_table, "output/mh_timecourse/deseq2/control_vs_m30_sig_degs.csv", col.names = TRUE, row.names = FALSE)
...
dedup_120_annots <- fread("output/mh_timecourse/deseq2/dedup_control_v_120m_sig_genes_with_annots.csv")

##DEGs missing at 240m
m120_vs_m240 <- data.table(setdiff(m120_names, m240_names))
missing_from_240 <- merge(m120_vs_m240, dedup_120_annots, by.x="V1", by.y="rn")
fwrite(missing_from_240, "output/mh_timecourse/deseq2/missing_from_240_annots.csv")

##DEGs with no annot
no_annot <- sig_w_annots[is.na(sig_w_annots$sprot_Top_BLASTX_hit),]
##make list of degs with no blast annot.
list_degs_no_annot <- data.table(no_annot$transcript_id)
##write list of degs with no annot.
fwrite(list_degs_no_annot, "output/mh_timecourse/deseq2/degs_with_no_annot.txt")

##blastx results
blastx_unann_degs <- fread("output/mh_timecourse/no_annot/blastx.outfmt6")
setnames(blastx_unann_degs, old=c("V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9", "V10", "V11", "V12", "V13"), new=c("transcript_id", "nr_db_id", "%_identical_matches", "alignment_length", "no_mismatches", "no_gap_openings", "query_start", "query_end", "subject_start", "subject_end", "evalue", "bit_score", "annotation"))
setorder(blastx_unann_degs, `transcript_id`, evalue, -bit_score)
##extract result with lowest evalue for each peptide - what if multiple rows with lowest min?
min_e_blastx_unann_degs <- blastx_unann_degs[,.SD[which.min(evalue)], by=`transcript_id`]
